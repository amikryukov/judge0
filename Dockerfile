...
ENV PATH "/usr/local/ruby-3.3.5/bin:/opt/.gem/bin:$PATH"
...
ENV PYTHON_VERSIONS 3.11.2

## bad place, but to save time
RUN set -xe && \
...
RUN mkdir -p /api/log && \
    ...
    chmod 0664 /api/log/development.log && \
    chown -R judge0: /api/log;
